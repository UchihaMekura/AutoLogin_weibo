...
class loginClient(object):
    ...
    def main(self, username, password):
        ...
        if login_result_dict['result'] == True:
            print("Login success!")
            self.loginSuccessFlag = 1
        else:
            print("Login Failed!")
            self.loginSuccessFlag = 0
...
def getCookies(id, pwd):

    client = loginClient(debug = True)

    cookie_dict = client.main(id,pwd)

    if client.loginSuccessFlag == 1:
